...
void KeyPair::New(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    if (args.IsConstructCall()) {
        if (args.Length() == 2) {
            ...
            ecrypt_status_t status = ECRYPT_FAIL;
            size_t private_key_length, public_key_length;
            status = ecrypt_gen_ec_key_pair(NULL, &private_key_length, NULL, &public_key_length);
            if (status != ECRYPT_BUFFER_TOO_SMALL) {
            ...
            status = ecrypt_gen_ec_key_pair(&prk[0], &private_key_length, &puk[0], &public_key_length);
            if (status != ECRYPT_SUCCESS) {
                ThrowError("Key Pair generation failed", status);
                args.GetReturnValue().SetUndefined();
            }
...
ecrypt_status_t ValidateKey(const std::vector<uint8_t>& key)
{
    if (key.empty()) {
        return false;
    }
    return ecrypt_is_valid_asym_key(&key[0], key.size());
}

bool IsValidKey(const std::vector<uint8_t>& key)
{
    return ValidateKey(key) == ECRYPT_SUCCESS;
}

bool IsPrivateKey(const std::vector<uint8_t>& key)
{
    if (!key.empty()) {
        ecrypt_key_kind_t kind = ecrypt_get_asym_key_kind(&key[0], key.size());
        switch (kind) {
...
{
    // If not invoked as "new ecrypt.SymmetricKey(...)" then reinvoke.
    if (!args.IsConstructCall()) {
    ...
    // If invoked as "new ecrypt.SymmetricKey()" then generate a new key.
    if (args.Length() == 0) {
        std::vector<uint8_t> buffer;

        size_t length = 0;
        ecrypt_status_t status = ecrypt_gen_sym_key(NULL, &length);
        if (status != ECRYPT_BUFFER_TOO_SMALL) {
            ThrowError("Ecrypt SymmetricKey", status);
            args.GetReturnValue().SetUndefined();
            return;
        }

        buffer.resize(length);
        status = ecrypt_gen_sym_key(&buffer.front(), &length);
        if (status != ECRYPT_SUCCESS) {
            ThrowError("Ecrypt SymmetricKey", status);
            args.GetReturnValue().SetUndefined();
            return;
    ...
    // If invoked as "new ecrypt.SymmetricKey(value)" then value must be
    ...
    if (node::Buffer::Length(value) == 0) {
        ThrowParameterError("Ecrypt SymmetricKey", "key is empty");
        args.GetReturnValue().SetUndefined();
        return;
    }
...
//
// Currently "new ecrypt.SymmetricKey()" produces instances of Buffer.
// This works in practice (because JavaScript), but it may be unexpected
// as "key instanceof ecrypt.SymmetricKey" returns false.
//
