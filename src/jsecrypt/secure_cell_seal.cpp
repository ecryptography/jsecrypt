...
{
    ecconnect_wipe(key_.data(), key_.size());
}
...
void SecureCellSeal::encrypt(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    status = ecrypt_secure_cell_encrypt_seal(&(obj->key_)[0],
                                             obj->key_.size(),
                                             context,
                                             context_length,
...
void SecureCellSeal::decrypt(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    status = ecrypt_secure_cell_decrypt_seal(&(obj->key_)[0],
                                             obj->key_.size(),
                                             context,
                                             context_length,
                                             (const uint8_t*)(node::Buffer::Data(args[0])),
...
{
    ecconnect_wipe(m_passphrase.data(), m_passphrase.size());
}
...
void SecureCellSealWithPassphrase::encrypt(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    status = ecrypt_secure_cell_encrypt_seal_with_passphrase(self->m_passphrase.data(),
    ...
    if (!encrypted_data) {
        ThrowError("SecureCellSeal.encrypt", ECRYPT_FAIL);
        args.GetReturnValue().SetUndefined();
        return;
    }
...
void SecureCellSealWithPassphrase::decrypt(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    status = ecrypt_secure_cell_decrypt_seal_with_passphrase(self->m_passphrase.data(),
                                                             self->m_passphrase.size(),
                                                             context_data,
                                                             context_length,
                                                             message_data,
    ...
    if (!decrypted_data) {
        ThrowError("SecureCellSeal.decrypt", ECRYPT_FAIL);
        args.GetReturnValue().SetUndefined();
        return;
    }
