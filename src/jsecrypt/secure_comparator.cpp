    ...
    : comparator_(NULL)
{
    ...
    if (!comparator_) {
        ThrowError("Secure Comparator constructor", ECRYPT_FAIL);
        return;
    }
    ecrypt_status_t status = secure_comparator_append_secret(comparator_, &secret[0], secret.size());
    if (ECRYPT_SUCCESS != status) {
...
void SecureComparator::beginCompare(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    if (ECRYPT_BUFFER_TOO_SMALL != status) {
    ...
    if (ECRYPT_SCOMPARE_SEND_OUTPUT_TO_PEER != status) {
        ThrowSecureComparatorError("Secure Comparator failed to begin comparison", status);
        free(data);
        args.GetReturnValue().SetUndefined();
        return;
...
{
    SecureComparator* obj = Nan::ObjectWrap::Unwrap<SecureComparator>(args.This());
    ecrypt_status_t res = secure_comparator_get_result(obj->comparator_);
    if (ECRYPT_SCOMPARE_NOT_READY == res) {
        ThrowSecureComparatorError("Secure Comparator not ready", ECRYPT_SCOMPARE_NOT_READY);
        args.GetReturnValue().SetUndefined();
        return;
    }
    args.GetReturnValue().Set(Nan::New<v8::Boolean>(ECRYPT_SCOMPARE_NO_MATCH != res));
}
...
void SecureComparator::isCompareComplete(const Nan::FunctionCallbackInfo<v8::Value>& args)
{
    ...
    args.GetReturnValue().Set(Nan::New<v8::Integer>(
        ECRYPT_SCOMPARE_NOT_READY != secure_comparator_get_result(obj->comparator_)));
}
