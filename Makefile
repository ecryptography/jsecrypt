...
# Increment VERSION when making a new release of ecrypt.
#
...
# Add ecrypt source directory to search paths
CFLAGS  += -I$(INC_PATH) -I$(SRC_PATH) -I$(SRC_PATH)/wrappers/ecrypt/
LDFLAGS += -L$(BIN_PATH)
...
CRYPTO_ENGINE = $(SRC_PATH)/ecconnect/$(CRYPTO_ENGINE_PATH)
...
ifeq ($(RSA_KEY_LENGTH),1024)
	CFLAGS += -Decrypt_RSA_KEY_LENGTH=RSA_KEY_LENGTH_1024
endif
ifeq ($(RSA_KEY_LENGTH),2048)
	CFLAGS += -Decrypt_RSA_KEY_LENGTH=RSA_KEY_LENGTH_2048
endif
ifeq ($(RSA_KEY_LENGTH),4096)
	CFLAGS += -Decrypt_RSA_KEY_LENGTH=RSA_KEY_LENGTH_4096
endif
ifeq ($(RSA_KEY_LENGTH),8192)
	CFLAGS += -Decrypt_RSA_KEY_LENGTH=RSA_KEY_LENGTH_8192
endif

ifeq ($(WITH_EXPERIMENTAL_OPENSSL_3_SUPPORT),yes)
	CFLAGS += -Decrypt_EXPERIMENTAL_OPENSSL_3_SUPPORT=1
endif
...
# FIXME(ilammy, 2020-10-29): enable stack canaries for Wasmecrypt too
...
# Binary format compatibility with ecrypt 0.9.6 on x86_64 architecture.
# https://github.com/cossacklabs/ecrypt/pull/279
# ecrypt 0.9.6 is going EOL on 2020-12-13 so it can be removed after that.
...
#include src/ecconnect/ecconnect.mk
#include src/ecrypt/ecrypt.mk
ifndef CARGO
include src/jsecrypt/jsecrypt.mk
#include src/wrappers/ecrypt/ecryptpp/ecryptpp.mk
#include src/wrappers/ecrypt/wasm/wasmecrypt.mk
#include jni/ecrypt_jni.mk
...
all: ecrypt_static ecconnect_static ecrypt_shared ecconnect_shared
	@echo $(VERSION)

ecconnect_static:  $(BIN_PATH)/$(LIBECCONNECT_A)
ecconnect_shared:  $(BIN_PATH)/$(LIBECCONNECT_SO)
ecrypt_static: $(BIN_PATH)/$(LIBecrypt_A)
ecrypt_shared: $(BIN_PATH)/$(LIBecrypt_SO)
ecrypt_jni:    $(BIN_PATH)/$(LIBecryptJNI_SO)

#ecconnect_pkgconfig:  $(BIN_PATH)/libecconnect.pc
#ecrypt_pkgconfig: $(BIN_PATH)/libecrypt.pc
...
for-audit: $(ECCONNECT_AUD) $(ecrypt_AUD)
...
IS_CENTOS := true
LD_SO_CONF = $(DESTDIR)/etc/ld.so.conf.d/ecrypt.conf
endif
endif

install: all install_ecconnect install_ecrypt
	@echo -n "ecrypt installed to $(PREFIX)"
	@$(PRINT_OK_)
...
ifdef IS_LINUX
	...
	@if [ -e /usr/include/ecrypt/ecrypt.h ] && [ -e /usr/local/include/ecrypt/ecrypt.h ]; then \
	     echo ""; \
	     echo "Multiple ecrypt installations detected in standard system paths:"; \
	     echo ""; \
	     echo "  - /usr"; \
	     echo "  - /usr/local"; \
	     echo ""; \
	     echo "This may lead to surprising behaviour when building and using software"; \
	     echo "which depends on ecrypt."; \
	     echo ""; \
	     echo "If you previously had ecrypt installed from source to \"/usr\","; \
...
uninstall: uninstall_ecrypt uninstall_ecconnect
	@echo -n "ecrypt uninstalled from $(PREFIX) "
	@$(PRINT_OK_)
...
########################################################################
#
# ecrypt distribution tarball
#

DIST_DIR = ecrypt_$(VERSION)

# ecrypt Core source code, tests, docs
#DIST_FILES += docs src tests Makefile VERSION
#DIST_FILES += README.md CHANGELOG.md LICENSE
#DIST_FILES += PKGBUILD.MSYS2 ecrypt.nsi
...
phpecrypt_install: CMD = cd src/wrappers/ecrypt/$(PHP_FOLDER) && phpize && ./configure && make install
phpecrypt_install:
ifdef PHP_VERSION
	@echo -n "phpecrypt install "
	@$(BUILD_CMD_)
else
...
ifneq ("$(wildcard src/wrappers/ecrypt/php/Makefile)","")
PHP_ecrypt_INSTALL = 1
endif

phpecrypt_uninstall: CMD = if [ -e src/wrappers/ecrypt/php/Makefile ]; then cd src/wrappers/ecrypt/php && make distclean ; fi;
phpecrypt_uninstall:
ifdef PHP_ecrypt_INSTALL
	@echo -n "phpecrypt uninstall "
	@$(BUILD_CMD_)
endif

uninstall: phpecrypt_uninstall

## Ruby ########################

rbecrypt_install: CMD = cd src/wrappers/ecrypt/ruby && gem build rbecrypt.gemspec && gem install ./*.gem $(_GEM_INSTALL_OPTIONS)
rbecrypt_install:
ifdef RUBY_GEM_VERSION
	@echo -n "rbecrypt install "
	@$(BUILD_CMD_)
else
...
rbecrypt_uninstall: CMD = gem uninstall ecrypt
rbecrypt_uninstall:
ifdef RUBY_GEM_VERSION
	@echo -n "rbecrypt uninstall "
	@$(BUILD_CMD_)
endif

uninstall: rbecrypt_uninstall

## Python ######################

ifdef PIP_VERSION
PIP_ecrypt_INSTALL := $(shell pip freeze |grep ecrypt)
endif

pyecrypt_install: CMD = cd src/wrappers/ecrypt/python/ && python3 setup.py install --record files3.txt
pyecrypt_install:
ifeq ($(PYTHON3_VERSION),)
	@echo "python3 not found"
	@exit 1
endif
	@echo -n "pyecrypt install "
	@$(BUILD_CMD_)

########################################################################
#
# Packaging ecrypt Core: Linux distributions
#
...
DEB_DEPENDENCIES_ecryptPP = --depends "$(DEB_CANONICAL_DEV_PACKAGE_NAME) = $(VERSION)+$(OS_CODENAME)"
...
RPM_DEPENDENCIES_ecryptPP = --depends "$(RPM_CANONICAL_DEV_PACKAGE_NAME) = $(RPM_VERSION)-$(RPM_RELEASE_NUM)"
...
CANONICAL_PACKAGE_NAME         = libecrypt
DEB_CANONICAL_DEV_PACKAGE_NAME = $(CANONICAL_PACKAGE_NAME)-dev
RPM_CANONICAL_DEV_PACKAGE_NAME = $(CANONICAL_PACKAGE_NAME)-devel
ifeq ($(PACKAGE_EMBEDDED_BORINGSSL),yes)
PACKAGE_NAME = libecrypt-boringssl
else
PACKAGE_NAME = libecrypt
endif
DEB_DEV_PACKAGE_NAME = $(PACKAGE_NAME)-dev
RPM_DEV_PACKAGE_NAME = $(PACKAGE_NAME)-devel
DEB_ecryptPP_PACKAGE_NAME = libecryptpp-dev
RPM_ecryptPP_PACKAGE_NAME = libecryptpp-devel
JNI_PACKAGE_NAME = libecrypt-jni
...
RPM_SUMMARY = Data security library for network communication and data storage. \
	 ecrypt is a data security library, providing users with high-quality security \
	 services for secure messaging of any kinds and flexible data storage. ecrypt \
...
DEV_PACKAGE_FILES += $(includedir)/ecconnect/
DEV_PACKAGE_FILES += $(includedir)/ecrypt/
DEV_PACKAGE_FILES += $(pkgconfigdir)/
DEV_PACKAGE_FILES += $(libdir)/$(LIBECCONNECT_A)
DEV_PACKAGE_FILES += $(libdir)/$(LIBECCONNECT_LINK)
DEV_PACKAGE_FILES += $(libdir)/$(LIBecrypt_A)
DEV_PACKAGE_FILES += $(libdir)/$(LIBecrypt_LINK)

LIB_PACKAGE_FILES += $(libdir)/$(LIBECCONNECT_SO)
LIB_PACKAGE_FILES += $(libdir)/$(LIBecrypt_SO)

ecryptPP_PACKAGE_FILES += $(includedir)/ecryptpp/

JNI_PACKAGE_FILES += $(jnidir)/$(LIBecryptJNI_SO)
...
deb: install ecryptpp_install ecrypt_jni_install
	...
	@fpm --input-type dir \
		 --output-type deb \
		 --name $(DEB_ecryptPP_PACKAGE_NAME) \
		 --license $(LICENSE_NAME) \
		 --url '$(COSSACKLABS_URL)' \
		 --description '$(SHORT_DESCRIPTION)' \
		 --maintainer $(MAINTAINER) \
		 --package $(BIN_PATH)/deb/$(DEB_ecryptPP_PACKAGE_NAME)_$(NAME_SUFFIX) \
		 --architecture $(DEB_ARCHITECTURE) \
		 --version $(VERSION)+$(OS_CODENAME) \
		 $(DEB_DEPENDENCIES_ecryptPP) \
		 --deb-priority optional \
		 --after-install $(POST_INSTALL_SCRIPT) \
		 --after-remove $(POST_UNINSTALL_SCRIPT) \
		 --category $(PACKAGE_CATEGORY) \
		 $(foreach file,$(ecryptPP_PACKAGE_FILES),$(DESTDIR)/$(file)=$(file))
...
rpm: install ecryptpp_install ecrypt_jni_install
	@printf "ldconfig" > $(POST_INSTALL_SCRIPT)
	@printf "ldconfig" > $(POST_UNINSTALL_SCRIPT)
	...
	@fpm --input-type dir \
         --output-type rpm \
         --name $(RPM_ecryptPP_PACKAGE_NAME) \
         --license $(LICENSE_NAME) \
         --url '$(COSSACKLABS_URL)' \
         --description '$(SHORT_DESCRIPTION)' \
         --rpm-summary '$(RPM_SUMMARY)' \
         ...
         --after-install $(POST_INSTALL_SCRIPT) \
         --after-remove $(POST_UNINSTALL_SCRIPT) \
         $(RPM_DEPENDENCIES_ecryptPP) \
         --package $(BIN_PATH)/rpm/$(RPM_ecryptPP_PACKAGE_NAME)-$(NAME_SUFFIX) \
         --version $(RPM_VERSION) \
         --category $(PACKAGE_CATEGORY) \
         $(foreach file,$(ecryptPP_PACKAGE_FILES),$(DESTDIR)/$(file)=$(file))
...
########################################################################
#
# Packaging ecrypt Core: Windows (NSIS)
#

nsis_installer: $(BIN_PATH)/Installecrypt.exe

$(BIN_PATH)/Installecrypt.exe: FORCE
ifdef IS_MSYS
...
########################################################################
#
# Packaging PHP ecrypt: Linux distributions
#
...
PHP_PACKAGE_NAME:=libphpecrypt-php$(PHP_VERSION_FULL)
PHP_POST_INSTALL_SCRIPT:=./scripts/phpecrypt_postinstall.sh
PHP_PRE_UNINSTALL_SCRIPT:=./scripts/phpecrypt_preuninstall.sh
PHP_API:=$(shell php -i 2>/dev/null|grep 'PHP API'|sed 's/PHP API => //')
PHP_LIB_MAP:=./src/wrappers/ecrypt/$(PHP_FOLDER)/.libs/phpecrypt.so=/usr/lib/php/$(PHP_API)/
